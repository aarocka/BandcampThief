...
var downloadListener = function() {
	...
	calcProgress = function(album) {
		...
		tprogress.progressTracks = (tprogress.tracksCompleted / tprogress.totalTracks);
		tprogress.progressSize = (tprogress.sizeCompleted / tprogress.totalSize);
	}

	self.on('song', function(t, tlen, len) {
		self.downloads[t.album_id] = self.downloads[t.album_id] || {};
		self.downloads[t.album_id][t.track_id] = t;
	...
	self.on('finished', function(t) {
		self.downloads[t.album_id][t.track_id] = null;
