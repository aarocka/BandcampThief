...
var Downloader = function Downloader() {
  ...
  var DownloadListener = function() {
    ...
    self.on('catchup', function(album_id) {
      finished = self.progress[album_id].finished;
      io.sockets.emit(album_id + ' catchup', {
        finished: finished
      });
    });
    ...
    self.on('finished', function(t) {
      ...
      self.progress[t.album_id].finished[t.track_id] = t;
    });
  ...
  self.downloadSong = function downloadSong(track, albumInfo, bandInfo, retry) {
    ...
    req.on('response', function(res) {
      if (!(res.statusCode > 199 && res.statusCode < 300)) {
        ...
        process.nextTick(function() {
          self.downloadSong(track, albumInfo, bandInfo, true);
        });
        return;
      }
  ...
  self.downloadAlbum = function downloadAlbum(albumId) {
      ...
      helper.getBandInfo(albumInfo.band_id, function(bandInfo, error) {
        ...
        mkdir_p(util.format('%s/%s/%s', downloadLocation, bandInfo.name, albumInfo.title), function(err) {
          ...
          for (var i = 0, l = albumInfo.tracks.length; i < l; i++) {
            ...
            self.downloadSong(track, albumInfo, bandInfo);
          }
        });
      });
    });
